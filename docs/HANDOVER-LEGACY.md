...
- Index cards precede the main visualization. The hero is a deterministic Top 200 stock bubble field from the latest published KFSP snapshot, strictly `average_volume_50_sessions > 300000`, ordered by that provider field descending then ticker ascending. The field is documented by KFSP as average volume over 50 sessions; its provider unit remains unverified in this repository, so the UI does not claim shares or lots. 1D/1W/1M/1Y values come from KFSP metrics when present. Only one radar layer and the largest twelve bubbles animate; reduced-motion disables both.
