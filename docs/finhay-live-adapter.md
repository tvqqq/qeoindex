...
QeoIndex keeps research state and historical scanner state separate from live market state.
...
- Live market state: Finhay Remote MCP after the QeoIndex user completes FHSC OAuth.
...
- QeoIndex requests only read-only market tools in this adapter.
- The ChatGPT Finhay connector session is not reused by QeoIndex.

## UI states

- `AUTH_REQUIRED`: QeoIndex has no Finhay OAuth session; user can connect.
