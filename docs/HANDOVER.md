...
- The rating table defaults to Top 100, uses a sector dropdown, supports sorting on every visible column, and displays both stock/sector RRG states. Metric tooltips, row keyboard activation, and the nine-tab detail dialog match the KFSP watchlist contract. The shared KFSP catalog also owns the canonical Top 100 ticker array used by the market board.
