...
- The rating table defaults to Top 100 and fits its 11 core metrics without a forced horizontal minimum width. `Tất cả` renders aggregated sector rows (count, summed capitalization, averaged scores/returns, dominant sector RRG); a sector click drills into stocks. The detail dialog adds a documented five-axis QeoIndex heuristic plus real 1D/7D/30D snapshot tracking while retaining the nine KFSP metric groups. Missing historical dates are not interpolated. See `docs/insights-homepage.md` for formulas and contracts.
