...
function normalizeProviderRecord(record: Record<string, unknown>) {
  ...
  const overviewKeys = [
    "ticker", "sector", "exchange", "price", "price_change_pct", "kfsp_canslim_score",
    "kfsp_score_4m", "kfsp_price_potential", "kfsp_fair_value", "average_volume_50_sessions",
    "average_volume_50d", "market_cap_billion", "rs_short", "rs_medium", "rs_long",
    "rsi_14", "weekly_change_pct", "monthly_change_pct", "beta", "pe_ttm", "pb_ttm",
  ]
...
function findMetric(metrics: MetricGroups, key: string): JsonValue | undefined {
  for (const group of KFSP_GROUPS) {
    if (key in metrics[group.key]) return metrics[group.key][key]
  }
  return undefined
}
...
function score(value: JsonValue | undefined) {
  const parsed = numeric(value)
  return parsed != null && parsed >= 0 && parsed <= 100 ? parsed : null
}

function compositeScore(values: Array<number | null>) {
...
Deno.serve(async (req: Request) => {
  ...
  const created = await supabase.from("kfsp_rating_sync_runs").insert({
    id: syncRunId,
    as_of_date: asOfDate,
    status: "running",
  ...
  try {
    ...
    if (rows.some((row) => row.kfsp_composite_score == null && row.kfsp_score_4m == null && row.kfsp_canslim_score == null && row.kfsp_stock_rs_score == null)) {
      throw new Error("KFSP_FILTER_SCORE_MISSING")
    }

    await supabase.from("kfsp_rating_sync_runs").update({
      provider_row_count: rows.length,
      token_refreshed: tokenRefreshed,
    }).eq("id", syncRunId)

    for (let index = 0; index < rows.length; index += 100) {
      const staged = await supabase.from("kfsp_rating_staging").insert(rows.slice(index, index + 100))
      if (staged.error) throw new Error("KFSP_STAGING_WRITE_FAILED")
    }

    const published = await supabase.rpc("publish_kfsp_rating_snapshot", {
      p_sync_run_id: syncRunId,
      p_minimum_rows: minimumRows,
    })
    if (published.error) throw new Error("KFSP_SNAPSHOT_PUBLISH_FAILED")
    ...
    await supabase.from("kfsp_rating_sync_runs").update({
      ...
      error_message: "KFSP daily rating sync failed; inspect Edge Function logs for provider diagnostics.",
