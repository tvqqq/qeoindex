...
begin
  ...
  on conflict on constraint kfsp_manual_dispatch_runs_pkey do nothing;
...
comment on function public.qeo_dispatch_kfsp_job(text, uuid, text, text[], boolean, text) is
  'One-shot idempotent service-role-only dispatcher for KFSP rating/TTAI recovery jobs. Uses the PK constraint explicitly to avoid PL/pgSQL output-variable ambiguity.';
