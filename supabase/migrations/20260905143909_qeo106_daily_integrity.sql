...
--
-- Invariants:
-- 1. A Daily row cannot exist on a known Vietnam cash-market closed session.
-- 2. Proven historical closed-session rows are quarantined before deletion.
-- 3. QEO-98 can consume one canonical 200-ticker audit RPC as a release gate.
...
begin
  ...
  select s.is_trading_day
    into v_is_trading_day
  from public.market_trading_sessions s
  where s.session_date = v_session_date;

  -- Fail closed only for dates covered by the authoritative calendar. Unknown
  -- future dates are left to the application calendar until an official notice
  -- is promoted into this table.
...
with universe as (
  select
    upper(stock->>'ticker') as ticker,
    upper(coalesce(stock->>'exchange', '')) as exchange
  ...
  from universe u
  left join daily d on d.ticker = u.ticker
  group by u.ticker, u.exchange
), expected as (
  select b.ticker, s.session_date
  from bounds b
  join public.market_trading_sessions s
    on s.is_trading_day = true
   and s.session_date between b.first_session and b.last_session
), missing as (
  select e.ticker, e.session_date
  from expected e
  left join daily d
    on d.ticker = e.ticker
   and d.session_date = e.session_date
   and d.is_trading_day = true
  where d.ticker is null
), expected_counts as (
  select ticker, count(*)::bigint as expected_sessions
  from expected
  group by ticker
), missing_counts as (
  select
    ticker,
    count(*)::bigint as missing_expected_sessions,
...
select
  ...
  b.first_session,
  b.last_session,
  coalesce(d.valid_daily_rows, 0),
  coalesce(e.expected_sessions, 0),
  coalesce(m.missing_expected_sessions, 0),
  coalesce(m.missing_session_dates, array[]::date[]),
  coalesce(d.non_trading_persisted_rows, 0),
  coalesce(d.verified_no_trade_rows, 0),
