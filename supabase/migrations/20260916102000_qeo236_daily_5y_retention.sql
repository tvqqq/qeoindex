...
-- QEO-236: extend the existing service-role retention RPC rather than adding a
-- second public maintenance surface. Canonical Daily OHLCV is permanently
-- bounded to a rolling five-calendar-year window; no cold/archive copy exists.
...
begin
  ...
  -- QEO-236 owns canonical Daily age-pruning. Retain the entire cutoff date by
  -- deleting strictly older bars only; the cutoff is based on Vietnam-local day.
  ...
  v_metrics := v_metrics || jsonb_build_array(jsonb_build_object(
    ...
    'deletedRows', v_daily_deleted,
    'oldestRetainedAt', v_oldest_daily,
  ...
  -- AI Council LLM evidence is bounded telemetry/audit material. Preserve any
  -- still-pending debate and only prune terminal debate rows.
  ...
  -- truly orphaned so retention cannot erase canonical analytical outputs.
  ...
  -- retained while bounded raw evidence still points at their run id.
  ...
  -- QEO-27 publisher owns deletion of kfsp_rating_raw_evidence. QEO-21/QEO-236
  -- only monitors expiry so retention has a single deletion owner.
  ...
  return jsonb_build_object(
    ...
    'rawHistoryRetention', jsonb_build_object(
      ...
      'oldestRetainedAt', v_oldest_daily,
      'policy', 'rolling 5 calendar years',
      'detail', 'Canonical Daily OHLCV is permanently retained for rolling 5 calendar years; no cold/archive copy is required.'
    )
  );
end;
$function$;
...
comment on function public.qeo_run_safe_retention_cleanup(timestamptz) is
  'QEO-21/QEO-236 service-role-only idempotent retention cleanup. Canonical Daily OHLCV is bounded to rolling 5 calendar years; telemetry/staging policies remain unchanged.';
