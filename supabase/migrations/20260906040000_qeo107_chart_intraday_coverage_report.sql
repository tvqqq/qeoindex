...
as $$
  ...
  hot as (
    ...
    where h.base_resolution = '1m' and h.bar_time >= p_hot_cutoff
    group by h.ticker
  ),
  cold as (
    select m.ticker, count(*)::bigint as manifest_count, coalesce(sum(m.row_count), 0)::bigint as row_count,
      min(m.range_start) as first_bar_time, max(m.range_end) as last_bar_time
    from public.chart_ohlcv_cold_manifests m
    join requested r on r.ticker = m.ticker
    where m.base_resolution = '1m' and m.verified_at is not null
    group by m.ticker
  ),
  derived as (
    select h.ticker, count(*)::bigint as row_count, min(h.bar_time) as first_bar_time, max(h.bar_time) as last_bar_time
  ...
  attempts as (
    ...
    where p.base_resolution = '1m' and p.detail ->> 'workflow' = 'QEO-107'
    group by p.ticker
  )
  select r.ticker,
...
comment on function public.qeo_chart_intraday_coverage(text[], timestamptz) is
  'QEO-107 canonical five-session raw 1m Hot/Cold + derived-hourly coverage report with explicit provider-gap/failure telemetry.';
