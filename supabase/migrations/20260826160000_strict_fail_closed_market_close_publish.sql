...
-- Strict Fail-Closed Market Close Snapshot Publishing
-- Recreates publish_market_insight_snapshot with strict assertions:
-- 1. quality_status must be 'healthy' or 'degraded' (never 'failing')
-- 2. Staged daily must be exactly 1
-- 3. Staged indexes must contain all 4 required canonical codes (VNINDEX, VN30, HNX, UPCOM) with non-null value
-- 4. Staged sectors must be >= 1
...
begin
  ...
  -- Check all 4 canonical index rows exist with non-null value
  ...
  -- 7. Update sync run status to succeeded
