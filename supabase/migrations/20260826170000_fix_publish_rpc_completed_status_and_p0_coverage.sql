...
-- Forward-Only Fix: Update publish_market_insight_snapshot
-- 1. Sets status = 'completed' (satisfying check constraint status in ('running', 'completed', 'failed', 'skipped'))
-- 2. Strictly verifies required P0 endpoint_coverage keys in market_insight_sync_runs:
--    - canonical_indexes = true
--    - market_pulse_content = true
--    - ma_breadth = true
--    - sector_pulse = true
...
begin
  ...
  -- Verify P0 coverage in v_run.endpoint_coverage
  ...
  -- 7. Update sync run status to completed (valid check constraint)
