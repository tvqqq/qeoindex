...
export const AI_COUNCIL_PROMPT_IDENTITY_VERSION = "prompt-identity-v3-ticker-knowledge"
...
export interface AiCouncilPromptIdentityInput {
  ...
  tickerKnowledgeHash?: string | null
...
export function resolveAiCouncilPromptIdentityHash(
  stock: {
    ...
    tickerKnowledge?: { contextHash?: unknown }
  },
  promptVersion: string,
) {
  ...
  const tickerKnowledgeHash = hashString(stock.tickerKnowledge?.contextHash)
  const computedIdentity = buildAiCouncilPromptIdentityHash({
    ...
    ...(tickerKnowledgeHash ? { tickerKnowledgeHash } : {}),
  ...
  // A frozen Notion identity predates the first-class Research Reports and ticker-knowledge
  // layers. Reuse it only when it covers the exact current prompt inputs; any persisted
  // ready/empty advisory snapshot hash forces the current identity to be recomputed.
  // Unavailable or unpersisted advisory evidence is absent and cannot perturb identity.
