...
- Legacy summary-image recovery uses the dedicated durable `research_reports.image_backfill` workflow. It selects only reports with a persisted current analysis matching the report content hash, never re-downloads the PDF or re-runs report-analysis AI, is bounded to 1–100 reports per dispatch, skips an already-ready image for the same `analysis_id`, and isolates per-report image failures. Production dispatch is machine-authenticated at `POST /api/research-reports/images/backfill`.
- Production operators may dispatch that endpoint through the service-role-only `qeo_trigger_research_report_image_backfill(maxReports, reason)` Supabase function; Vault credentials remain inside the database boundary and are never returned to callers.
