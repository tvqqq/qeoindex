...
function activeTopics() {
  return [...topicListeners.entries()].filter(([, listeners]) => listeners.size > 0).map(([topic]) => topic)
}
...
function parseDataMessage(value: unknown): MarketRelayDataMessage | null {
  ...
  const frames = Array.isArray(source.frames)
    ? source.frames.filter((frame): frame is MarketRelayFrame => Boolean(frame) && typeof frame === "object" && !Array.isArray(frame))
    : []
  if (!epoch || !Number.isSafeInteger(sequence) || sequence <= 0) return null
  if (type === "market") {
    return { type, epoch, sequence, publishedAt, frames }
  }
  if (type === "orderbook") {
    const symbol = typeof source.symbol === "string" ? source.symbol.trim().toUpperCase() : ""
    if (!symbol) return null
    return { type, symbol, epoch, sequence, publishedAt, continuityGap: source.continuityGap === true, frames }
  }
  return null
}
...
export function subscribeMarketRelay(
  ...
  return () => {
    ...
    if (current && current.size === 0) {
      ...
      if (connectionState.status === "READY") send({ type: "unsubscribe", topics: [normalized] })
    }
