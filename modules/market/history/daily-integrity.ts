...
async function repairTicker(
  ...
  const rows = suspectDates.flatMap((sessionDate) => {
    ...
    if (!bar) return []
    // Generic fallback zero-volume bars are exactly the historical failure mode in QEO-106.
    // Preserve a missing/unclassified session unless a canonical authority returns evidence.
