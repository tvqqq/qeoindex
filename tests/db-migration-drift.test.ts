...
test("mapped timestamp mismatch passes only when explicitly declared", () => {
  const result = reconcileMigrations({
    ...
    manifest: {
      migrations: [{
        logicalName: "ai_council_authenticated_readonly",
        repositoryVersion: "20260902084500",
        productionVersion: "20260902014425",
        state: "MAPPED",
        evidence: "ledger-logical-name",
      }],
    },
  })
...
test("unexplained repo-only migration fails closed", () => {
  const result = reconcileMigrations({
    activeFiles: ["20260902090000_repo_only.sql"],
    pendingFiles: [],
    productionLedger: [],
    manifest: { migrations: [] },
  })
...
test("unexplained production-only migration fails closed", () => {
  const result = reconcileMigrations({
    activeFiles: [],
    pendingFiles: [],
    productionLedger: [{ version: "20260902090000", name: "prod_only" }],
    manifest: { migrations: [] },
  })
  assert.equal(result.ok, false)
  assert.match(result.errors.join("\n"), /production-only/i)
...
test("production-applied migration cannot remain pending", () => {
  const result = reconcileMigrations({
    ...
    manifest: {
      migrations: [{
        logicalName: "kfsp_rating_storage_refactor",
        repositoryVersion: "20260902090000",
        productionVersion: "20260902020424",
        state: "MAPPED",
        evidence: "production-schema-contract",
      }],
    },
  })
  assert.equal(result.ok, false)
