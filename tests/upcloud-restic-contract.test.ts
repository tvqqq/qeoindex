...
test("daily backup quiesces Hermes only for staging and backs up a relative tree", () => {
  ...
  const common = source("ops/upcloud/restic/common.sh")
  assert.match(common, /hermes-gateway-\*\.service/)
  assert.match(common, /systemctl --user/)
  assert.match(common, /runuser -u "\$HERMES_RUNTIME_USER"/)
  assert.match(common, /XDG_RUNTIME_DIR=\/run\/user\/\$uid/)
  assert.match(backup, /discover_hermes_gateway_unit/)
  assert.match(backup, /hermes_gateway_stop/)
