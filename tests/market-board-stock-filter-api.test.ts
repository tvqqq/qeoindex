...
test("market quote reconcile route is authenticated, canonical-bounded, and uses bounded fallbacks", () => {
  ...
  assert.match(quoteRoute, /const currentSessionDate = vietnamDateKey\(\)/)
  assert.match(quoteRoute, /const previousSessionDate = previousTradingDateKey\(currentSessionDate\)/)
  assert.match(quoteRoute, /snapshot\.session_date === currentSessionDate/)
