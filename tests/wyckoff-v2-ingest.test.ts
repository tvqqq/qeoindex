...
function completeSnapshot(ticker: string, timeframe: "1H" | "4H" | "1D" | "1W" | "1M", rank: number | null): WyckoffV2Snapshot {
  ...
  return {
    ...
    evidence: { provider: "DNSE", providerDetail: "cache", sourceUrl: "https://openapi.dnse.com.vn/history", fetchedAt: "2026-08-25T08:20:00.000Z", firstBarAt: "2025-01-01T00:00:00.000Z", lastBarAt: "2026-08-25T07:45:00.000Z", completedBars: 80, derived: timeframe !== "1H" && timeframe !== "1D", rulesTriggered: [], missingReason: "" },
    markers: [],
    scenarios: [
...
function fullSet() {
  ...
  const monthly = rows.find((row) => row.ticker === "T100" && row.timeframe === "1M")!
  monthly.historyBarCount = 24
  monthly.historyStatus = "Incomplete"
  monthly.phase = null
  monthly.wyckoffState = null
  monthly.taBias = null
  monthly.confidence = null
  monthly.bullProbability = null
  monthly.baseProbability = null
  monthly.bearProbability = null
  monthly.support = null
  monthly.resistance = null
  monthly.confirmation = null
  monthly.invalidation = null
  monthly.whatChanged = null
  monthly.technical = {}
  monthly.markers = []
  monthly.scenarios = []
  monthly.evidence.completedBars = 24
  monthly.evidence.missingReason = "Only 24 completed bars; minimum 60 completed bars required for 1M."
  return rows
}
...
test("v2 membership validation keeps duplicate Rank anomalies as data, not ingest blockers", () => {
  ...
  assert.equal(memberships.length, 100)
  assert.equal(new Set(memberships.map((row) => row.ticker)).size, 100)
  assert.equal(memberships.filter((row) => row.rank === 21).length, 2)
})

test("v2 Supabase payload preserves all 500 snapshots including genuine Incomplete", () => {
  const snapshots = fullSet()
  const payload = buildWyckoffV2SupabasePayload({ snapshots, runId: "11111111-1111-4111-8111-111111111111", scanDate: "2026-08-25", runKey: "WYCKOFF-2026-08-25-EOD-v2" })
  assert.equal(payload.memberships.length, 100)
  assert.equal(payload.snapshots.length, 500)
