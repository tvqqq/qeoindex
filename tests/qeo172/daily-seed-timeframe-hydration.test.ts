...
test("QEO-241 active timeframes render deterministically from the bounded SSR Daily seed", () => {
  ...
  for (const timeframe of ["1D", "1W", "1M"] as ChartTimeframe[]) {
  ...
  assert.ok(derive(dailyBars, "1M").length < dailyBars.length, "monthly seed must be aggregated rather than relabeled Daily bars")
})
