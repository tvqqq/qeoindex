...
test("rating table exposes keyboard modal, grouped dashboard tabs, history, and sector interactions", () => {
  ...
  assert.match(insightsDashboard, /type StockDetailTab = "overview" \| "info" \| "fa" \| "ta" \| "kfsp"/)
  for (const tab of ["Tổng quan", "Thông tin", "Chỉ số FA", "Phân tích TA", "KFSP"]) {
    assert.match(insightsDashboard, new RegExp(`label: "${tab}"`))
  }
  for (const panel of ["overview", "info", "fa", "ta", "kfsp"]) {
    assert.match(insightsDashboard, new RegExp(`topTab === "${panel}"`))
    assert.match(insightsDashboard, new RegExp(`rating-panel-${panel}`))
  }
  ...
  assert.match(insightsDashboard, /kfsp_stock_rs_score/)
  assert.match(insightsDashboard, /kfsp_sector_rs_score/)
  assert.match(insightsDashboard, /kfsp_stock_rrg_state/)
  assert.match(insightsDashboard, /kfsp_sector_rrg_state/)
