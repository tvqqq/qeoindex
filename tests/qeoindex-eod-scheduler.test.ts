...
test("QeoIndex EOD scheduler uses one 15:15 ICT pg_cron job with Vault-backed machine auth", () => {
  ...
  assert.match(sql, /create or replace function public\.qeo_verify_eod_scheduler_secret\(p_secret text\)/i)
  assert.match(sql, /grant execute on function public\.qeo_verify_eod_scheduler_secret\(text\) to service_role/i)
  assert.match(sql, /revoke all on function public\.qeo_verify_eod_scheduler_secret\(text\) from public, anon, authenticated/i)
  assert.match(sql, /return v_cron_secret = p_secret/i)
