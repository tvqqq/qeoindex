...
test("Legacy drawing with missing time or price is NEVER guessed and marked unresolved", () => {
  const unsafeDrawings = [
    { id: "legacy-missing-time", tool: "horizontal", points: [{ x: 200, y: 300, price: 50 }] },
    { id: "legacy-missing-price", tool: "text", points: [{ x: 150, y: 250, time: 1700000000 }], text: "Note" },
    { id: "legacy-pixels-only", tool: "trendline", points: [{ x: 100, y: 200 }, { x: 300, y: 400 }] },
  ]
...
test("QEO-241 migrateDrawings normalizes retired V2 source timeframes to 1D", () => {
  for (const sourceTimeframe of ["1h", "3D", "1Q", "1Y"]) {
    const existingV2 = {
      schemaVersion: 2,
      id: `existing-v2-${sourceTimeframe}`,
      tool: "arrow",
      anchors: [
    ...
    assert.equal(result.migrated.length, 1, `${sourceTimeframe} must migrate instead of being discarded`)
...
test("backupLegacyLocalSettings preserves backup under expected key and is idempotent", () => {
  ...
  try {
    ...
    const rawLegacyPayload = JSON.stringify({ ticker: "FPT", drawings: [{ id: "draw-old", x: 10, y: 20 }] })
    assert.equal(backupLegacyLocalSettings(ticker, rawLegacyPayload), true)
    assert.equal(store.get(backupKey), rawLegacyPayload)
    assert.equal(backupLegacyLocalSettings(ticker, JSON.stringify({ ticker: "FPT", drawings: [] })), false)
    assert.equal(store.get(backupKey), rawLegacyPayload)
  } finally {
