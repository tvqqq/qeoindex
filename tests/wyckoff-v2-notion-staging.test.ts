...
function snapshot(ticker: string, timeframe: "1H" | "4H" | "1D" | "1W" | "1M", rank: number | null): WyckoffV2Snapshot {
  ...
  return {
    snapshot: `${ticker} · ${timeframe} · 2026-08-25`,
    snapshotKey: `${runKey}|${ticker}|${timeframe}`,
    runKey,
    ticker,
    rank,
    exchange: "HOSE",
    sector: "Consumer",
    timeframe,
    barClosedAt: "2026-08-25T07:45:00.000Z",
    historyBarCount: 80,
    historyStatus: "Complete",
    provider: "DNSE",
    providerDetail: "DNSE cached history",
    sourceUrl: `https://openapi.dnse.com.vn/price/ohlc?symbol=${ticker}&resolution=1D&type=STOCK`,
    fetchedAt: "2026-08-25T08:20:00.000Z",
    modelVersion: "qeo-wyckoff-rule-v1",
    aggregationVersion: "vn-session-v1",
    promptVersion: "notion-unified-v2",
    phase: "Markup / Reaccumulation watch",
    wyckoffState: "Cấu trúc tăng nhưng chưa có event xác nhận mới.",
    taBias: "Bullish",
    confidence: "LOW",
    bullProbability: 40,
    baseProbability: 35,
    ...
    support: "60",
    resistance: "70",
    confirmation: "Breakout → Hold → Retest → Follow-through.",
    invalidation: "Acceptance dưới Support 60.",
    whatChanged: "Baseline scan.",
    ...
    evidence: {
      ...
      lastBarAt: "2026-08-25T07:45:00.000Z",
      completedBars: 80,
      derived: timeframe !== "1H" && timeframe !== "1D",
      rulesTriggered: ["Above MA50"],
      missingReason: "",
    },
    markers: [],
    scenarios: [
...
test("v2 snapshot set validates exactly 500 unique keys while rank anomalies remain non-blocking", () => {
  const snapshots = fullSet()
  const result = validateWyckoffV2SnapshotSet("WYCKOFF-2026-08-25-EOD-v2", snapshots)
  assert.equal(result.total, 500)
  assert.equal(result.complete, 500)
  assert.equal(result.incomplete, 0)
  assert.equal(result.invalid, 0)

  const duplicate = snapshots.slice()
  duplicate[499] = duplicate[0]
  assert.throws(() => validateWyckoffV2SnapshotSet("WYCKOFF-2026-08-25-EOD-v2", duplicate), /duplicate|500 unique/i)
})
...
test("run property mapping declares v2 ownership and actual counters", () => {
  const properties = buildWyckoffV2RunProperties({
    ...
    providerSummary: "QeoIndex server writer · persistent OHLCV cache",
    validationHash: "",
  }) as Record<string, any>
  assert.equal(properties.Run.title[0].text.content, "WYCKOFF-2026-08-25-EOD-v2")
  assert.equal(properties.Status.select.name, "Writing")
  assert.equal(properties["Snapshot Expected"].number, 500)
