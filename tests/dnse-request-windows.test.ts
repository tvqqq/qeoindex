...
test("QEO-225 authenticates relay before subscribing while Supabase is bootstrap-only", () => {
  ...
  assert.doesNotMatch(orderbook, /supabase\.realtime|private:\s*true|broadcast|\.channel\(/)
})
...
test("centralized worker keeps durable checkpoints bounded while relay owns live delivery", () => {
...
test("QEO-225 removes browser-direct provider orderbook transport and Supabase Broadcast hot path", () => {
  ...
  assert.doesNotMatch(transport, /private:\s*true|broadcast|\.channel\(/)
