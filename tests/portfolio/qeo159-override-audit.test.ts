...
test("persisted planned Trade uses an isolated QEO-137 mutation boundary with auditable concentration override", () => {
  const advisor = read("components/portfolio/risk-sizing/trade-size-advisor.tsx")
  const persistence = read("components/portfolio/risk-sizing/planned-trade-persistence.tsx")

  assert.match(advisor, /PlannedTradePersistence/)
  assert.match(advisor, /portfolioId=\{portfolioId\}/)
  assert.match(advisor, /trade=\{draftPlannedTrade\}/)
  assert.match(advisor, /requiresConcentrationOverride=\{requiresConcentrationOverride\}/)
  ...
  assert.match(persistence, /trade:\s*PlannedTrade \| null/)
  assert.match(persistence, /selectedMode/)
  assert.match(persistence, /"live"/)
  assert.match(persistence, /"paper"/)
  assert.match(persistence, /overrideReason/)
  assert.match(persistence, /requiresConcentrationOverride/)
  assert.match(persistence, /overrideReason\.trim\(\)/)
  assert.match(persistence, /\/api\/portfolio\/\$\{portfolioId\}\/trades/)
  assert.match(persistence, /money_management_plan_id:\s*riskContext\.moneyManagementPlanId \?\? null/)
  ...
  assert.match(persistence, /override_reason:\s*overrideReason\.trim\(\)/)
  assert.match(persistence, /Không thể lưu bằng chứng override/)
  assert.match(persistence, /Lưu Trade dự kiến/)
})
