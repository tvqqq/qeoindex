...
test("only the 7 allowlisted jobs are manual-safe", () => {
  ...
  assert.equal(isManualJobAllowed("market.sync_eod"), false, "retired final market sync must stay owned by canonical EOD")
...
test("KFSP manual recovery is confirmation-gated and wired to the one-shot RPC", () => {
  ...
  assert.match(jobs, /definition\?\.manualPolicy === "confirm"/)
