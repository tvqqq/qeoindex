...
test("QEO-190 wires the selected PE/PB metric and history window into a compact valuation snapshot", () => {
  const view = read("components/insights/market-health-view.tsx")

  assert.match(view, /VALUATION_HISTORY_RANGES[\s\S]*1Y[\s\S]*3Y[\s\S]*5Y[\s\S]*Tất cả/)
  assert.match(view, /valuationRangeYears/)
  assert.match(view, /buildValuationSnapshot\([\s\S]*metric:\s*valuationMetric[\s\S]*history:\s*data\.dailySummary\.valuationHistory[\s\S]*rangeYears:\s*valuationRangeYears/)
  assert.match(view, /visibleValuationSeries/)
  assert.match(view, /<ValuationSnapshot[\s\S]*snapshot=\{valuationSnapshot\}/)
  assert.match(view, /Hiện tại/)
  assert.match(view, /Trung vị/)
  assert.match(view, /Trung bình/)
  assert.match(view, /Percentile/)
  assert.match(view, /σ so với TB/)
  assert.match(view, /Chưa đủ dữ liệu/)
  assert.doesNotMatch(view, /mục tiêu giá|target price|upside|downside/i, "valuation snapshot must stay descriptive rather than become a price forecast")
})
