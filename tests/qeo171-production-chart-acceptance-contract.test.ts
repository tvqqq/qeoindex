...
test("QEO-171 has an isolated authenticated production browser acceptance harness", () => {
  ...
  assert.match(workflow, /QEO171_TEST_EMAIL:\s*\$\{\{\s*secrets\.QEO144_TEST_EMAIL\s*\}\}/)
  assert.match(workflow, /QEO171_TEST_PASSWORD:\s*\$\{\{\s*secrets\.QEO144_TEST_PASSWORD\s*\}\}/)
  assert.match(workflow, /playwright install --with-deps chromium/)
  assert.match(workflow, /qeo171-stock-chart-production\.spec\.ts --project=desktop/)
  assert.doesNotMatch(workflow, /SUPABASE_SERVICE_ROLE_KEY|glwhhrmejlonhyorvtzm/)
