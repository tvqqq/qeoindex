...
export interface CouncilLlmEvidenceContext {
  ...
  ttaiQuarterlyHistory: Array<{
    ...
    score4m: number | null
    canslimScore: number | null
    fourmComponents: Record<string, unknown>
    canslimComponents: Record<string, unknown>
    source: string
    fetchedAt: string
  }>
  wyckoffMtf: Array<{
...
function contextHash(context: CouncilLlmEvidenceContext) {
...
function selectProviderMetrics(metrics: unknown) {
  ...
  return Object.fromEntries(
    PROVIDER_GROUPS
      .filter((group) => root[group] != null)
      .map((group) => [group, record(root[group])]),
  )
}
...
function normalizedHistoryRow(row: TtaiHistoryRow) {
  return {
    ...
    score4m: nullableNumber(row.fourm_score),
    canslimScore: nullableNumber(row.canslim_score),
    fourmComponents: record(row.fourm_components),
    canslimComponents: record(row.canslim_components),
    source: row.source || "kfsp",
...
function normalizedWyckoffRow(row: WyckoffEvidenceRow) {
  return {
    ...
    technical: record(row.technical),
...
function buildContext(params: {
  ...
  ratingDate: string
  rating: RatingEvidenceRow | null
  ttaiRows: TtaiHistoryRow[]
  wyckoffRows: WyckoffEvidenceRow[]
}): CouncilLlmEvidenceContext {
  ...
  return {
    ...
    ticker: stock.ticker,
    providerSnapshot,
    ttaiQuarterlyHistory: ttaiRows
      .sort((left, right) => Number(right.period_year) - Number(left.period_year) || Number(right.period_quarter) - Number(left.period_quarter))
      .slice(0, TTAI_HISTORY_LIMIT)
      .map(normalizedHistoryRow),
    wyckoffMtf: wyckoffRows
      .sort((left, right) => (timeframeRank.get(left.timeframe) ?? 99) - (timeframeRank.get(right.timeframe) ?? 99))
      .map(normalizedWyckoffRow),
...
async function loadRatingEvidence(supabase: SupabaseClient, ratingDate: string, tickers: string[]) {
  ...
  const result = await supabase
    ...
    .select("ticker,as_of_date,company_name,sector,exchange,top100_rank,price,price_change_pct,kfsp_composite_score,kfsp_score_4m,kfsp_canslim_score,kfsp_price_potential,kfsp_stock_rs_score,kfsp_sector_rs_score,rs_short,rs_medium,weekly_change_pct,monthly_change_pct,beta,pe_ttm,pb_ttm,kfsp_metrics")
  ...
  return new Map(((result.data || []) as RatingEvidenceRow[]).map((row) => [row.ticker, row]))
}
...
async function loadTtaiEvidence(supabase: SupabaseClient, tickers: string[]) {
  ...
  for (let offset = 0; offset < tickers.length; offset += TTAI_QUERY_BATCH_SIZE) {
    ...
    const result = await supabase
      ...
      .select("ticker,period,period_year,period_quarter,fourm_score,canslim_score,fourm_components,canslim_components,source,fetched_at")
      .in("ticker", batch)
...
async function loadWyckoffEvidence(supabase: SupabaseClient, tickers: string[]) {
  ...
  const result = await supabase
    ...
    .in("ticker", tickers)
    .in("timeframe", [...WYCKOFF_TIMEFRAMES])
...
async function loadPersistedContexts(supabase: SupabaseClient, runIds: string[]) {
  ...
  const result = await supabase
    ...
    .select("run_id,ticker,context_hash,context_version,context_payload,source_limitations")
    .in("run_id", runIds)
...
export async function enrichCouncilStocksWithLlmEvidence(
  ...
  if (!ratingDate || !stocks.length) {
    return {
      ...
      contextsReused: 0,
      contextsPersisted: 0,
      missingRunIdentities: stocks.length,
      ttaiRowsLoaded: 0,
      wyckoffRowsLoaded: 0,
      detail: "P4.3 evidence hydration skipped because no current Council rating date/stock set is available.",
    }
  }
  ...
  for (const stock of stocks) {
    ...
    const context = buildContext({
      stock,
      ratingDate,
      rating: ratings.get(stock.ticker) || null,
      ttaiRows: [...(ttai.byTicker.get(stock.ticker) || [])],
      wyckoffRows: [...(wyckoff.byTicker.get(stock.ticker) || [])],
    })
    ...
    pendingRows.push({
      ...
      context_version: AI_COUNCIL_LLM_EVIDENCE_VERSION,
      context_hash: hash,
      context_payload: context,
      source_limitations: context.sourceLimitations,
    })
  }
  ...
  if (pendingRows.length) {
    const persist = await supabase
      .from("ai_council_llm_evidence")
      .upsert(pendingRows, { onConflict: "run_id", ignoreDuplicates: true })
