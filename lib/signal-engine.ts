...
export function parsePriceLevels(text: string) {
...
export function marketSessionProgress(timestampMs = Date.now()) {
  ...
  const elapsed = clock <= morningEnd
    ? clock - morningStart
    : (morningEnd - morningStart) + (clock - afternoonStart)
...
export function estimateVolumePace(scan: SignalDailyScan, currentVolume: number, timestampMs = Date.now()) {
  ...
  const expected = Math.max(1, baseline * session.progress)
  return currentVolume / expected
}

function nearestBelow(levels: number[], price: number) {
  return levels.filter((level) => level < price).sort((a, b) => b - a)[0] ?? null
}

function nearestAbove(levels: number[], price: number) {
...
export function evaluateBuy(scan: SignalDailyScan, quote: LiveQuote, timestampMs = Date.now()): BuyDecision {
  ...
  const pace = estimateVolumePace(scan, quote.totalVolume, timestampMs)
  ...
  const checks = [
    ...
    aboveMa50,
    pace != null && pace >= 1.35,
    !scan.atr14 || atrDistance <= 2.2,
    breakout || momentum,
  ]
  if (!checks.every(Boolean)) {
    const why = [
      changePct < 0.8 ? "momentum < +0.8%" : null,
      changePct > 5.5 ? "giá đã tăng > +5.5%" : null,
      !aboveMa20 ? "dưới MA20" : null,
      !aboveMa50 ? "dưới MA50" : null,
      pace == null || pace < 1.35 ? `volume pace ${pace == null ? "N/A" : pace.toFixed(2) + "x"} < 1.35x` : null,
      scan.atr14 && atrDistance > 2.2 ? `giá cách prior close ${atrDistance.toFixed(2)} ATR` : null,
      !breakout && !momentum ? "chưa breakout/đủ momentum" : null,
    ].filter(Boolean).join("; ")
  ...
  return {
    signal: true,
    reason: `BUY v1: Daily Bullish ${scan.bullProbability ?? "—"}/${scan.baseProbability ?? "—"}/${scan.bearProbability ?? "—"}; ${trigger}; volume pace ${pace?.toFixed(2)}x; giữ trên MA20/MA50; stop ${stopPrice.toFixed(2)} (${riskPct.toFixed(2)}%).`,
    stopPrice,
    targetPrice,
    riskPct,
    volumePace: pace,
...
export function evaluateExit(open: OpenRecommendationState, scan: SignalDailyScan | undefined, quote: LiveQuote, timestampMs = Date.now()): ExitDecision {
  ...
  if (quote.price <= open.stopPrice) {
    return { signal: true, type: "EXIT_FAIL", reason: `Hard stop: giá ${quote.price.toFixed(2)} <= stop ${open.stopPrice.toFixed(2)}.`, volumePace: pace, returnPct, maxFavorablePct, maxAdversePct }
  }
  if (scan && scan.taBias !== "Bullish") {
    return { signal: true, type: "EXIT_FAIL", reason: `Daily thesis fail: Bias chuyển ${scan.taBias}; không còn điều kiện Bullish ban đầu.`, volumePace: pace, returnPct, maxFavorablePct, maxAdversePct }
  }
  if (scan?.ma20 && quote.price < scan.ma20 && pace != null && pace >= 1.25) {
    return { signal: true, type: "EXIT_FAIL", reason: `Structural fail: giá mất MA20 ${scan.ma20.toFixed(2)} với volume pace ${pace.toFixed(2)}x.`, volumePace: pace, returnPct, maxFavorablePct, maxAdversePct }
  }
  const support = scan ? nearestBelow(parsePriceLevels(scan.support), open.buyPrice) : null
  if (support && quote.price < support && pace != null && pace >= 1.2) {
    return { signal: true, type: "EXIT_FAIL", reason: `Acceptance risk dưới support ${support.toFixed(2)} với volume pace ${pace.toFixed(2)}x.`, volumePace: pace, returnPct, maxFavorablePct, maxAdversePct }
  }
