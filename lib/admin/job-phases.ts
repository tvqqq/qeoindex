...
export interface SystemJobPhaseRow {
  id: string; run_id: string; job_key: string; phase_key: string; phase_order: number; status: string; started_at: string
  finished_at?: string | null; duration_ms?: number | null; summary?: Record<string, unknown> | null; error_code?: string | null; error_message?: string | null; created_at?: string
}
export interface AdminJobPhaseView {
  key: QeoIndexEodPhaseKey; order: number; businessPhase: QeoIndexEodBusinessPhaseKey; label: string; description: string; status: AdminJobPhaseStatus
  startedAt: string | null; finishedAt: string | null; durationMs: number | null; summary: Record<string, unknown> | null; errorCode: string | null; errorMessage: string | null
}
...
function rowTimestamp(row: SystemJobPhaseRow) { return row.finished_at || row.started_at || row.created_at || "" }
export function buildAdminJobPhaseTimeline(rows: SystemJobPhaseRow[]): AdminJobPhaseView[] {
  ...
  return QEOINDEX_EOD_PHASES.map((definition) => {
    ...
    return {
      key: definition.key, order: definition.order, businessPhase: definition.businessPhase, label: definition.label, description: definition.description,
      status: normalizePhaseStatus(row?.status), startedAt: row?.started_at ?? null, finishedAt: row?.finished_at ?? null, durationMs: row?.duration_ms ?? null,
      summary: row?.summary ?? null, errorCode: row?.error_code ?? null, errorMessage: row?.error_message ?? null,
    }
  })
}
