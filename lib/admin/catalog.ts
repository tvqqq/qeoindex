...
export const ADMIN_SETTING_CATALOG: AdminSettingDefinition[] = [
  ...
  },
  {
    ...
    validate(val: unknown): AdminValidationResult {
      ...
      return parsed !== null
        ? { ok: true, value: parsed }
        : { ok: false, error: "Giới hạn lịch sử job phải là số nguyên từ 20 đến 200" }
    },
  },
  {
...
export const ADMIN_JOB_CATALOG: AdminJobDefinition[] = [
  ...
  },
  {
    ...
    provider: "vercel_cron",
    label: "AI Council Daily Synthesis",
  ...
  },
  {
    ...
    group: "market",
    scheduleUtc: "*/5 2-8 * * 1-5",
    scheduleIct: "Mỗi 5p (09:00-15:00 T2-T6)",
    manualPolicy: "disabled",
    freshnessMinutes: 15,
  ...
  },
  {
    ...
    provider: "supabase_pg_cron",
    label: "KFSP TTAI History Hourly",
    description: "Cập nhật lịch sử dữ liệu TTAI hàng giờ.",
    group: "provider",
    scheduleUtc: "0 * * * *",
    scheduleIct: "Hàng giờ",
    manualPolicy: "disabled",
