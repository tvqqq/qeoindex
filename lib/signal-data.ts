...
function parseRecommendation(page: any): TradeRecommendation | null {
  ...
  return {
    ...
    notionUrl: page.url ?? "",
    ticker,
    status: select(props.Status) as TradeRecommendation["status"],
    buySignal: date(props["Buy Signal"]),
    buyPrice: number(props["Buy Price"]) ?? 0,
    buyReason: text(props["Buy Reason"]),
    stopPrice: number(props["Stop Price"]) ?? 0,
    riskPct: number(props["Risk Pct"]),
    targetPrice: number(props["Initial Target"]),
    sellSignal: date(props["Sell Signal"]),
    sellPrice: number(props["Sell Price"]),
    sellReason: text(props["Sell Reason"]),
    returnPct: number(props["Return Pct"]),
    vnindexEntry: number(props["VNINDEX Entry"]),
    vnindexExit: number(props["VNINDEX Exit"]),
    vnindexReturnPct: number(props["VNINDEX Return Pct"]),
    alphaPct: number(props["Alpha Pct"]),
    outcome: select(props.Outcome) as TradeRecommendation["outcome"],
    ...
    lastRelVolume: number(props["Last Rel Volume"]),
    maxFavorablePct: number(props["Max Favorable Pct"]),
    maxAdversePct: number(props["Max Adverse Pct"]),
  }
}
...
function parseEvent(page: any): SignalEventRow | null {
  ...
  return {
    ...
    signalTime: date(props["Signal Time"]),
    ...
    scanDate: date(props["Scan Date"]),
    dailyBias: select(props["Daily Bias"]),
    stopPrice: number(props["Stop Price"]),
    vnindex: number(props.VNINDEX),
  }
}
...
export async function createBuyRecommendation(args: { scan: DailyScanRow; quote: LiveQuote; decision: BuyDecision; vnindex: number | null }) {
  ...
  const properties: Record<string, any> = {
    Recommendation: { title: [{ type: "text", text: { content: `${args.scan.ticker} — BUY — ${now.slice(0, 16)}` } }] },
    Ticker: rich(args.scan.ticker),
    Status: { select: { name: "Open" } },
    "Buy Signal": { date: { start: now } },
    "Buy Price": num(args.quote.price),
    "Buy Reason": rich(args.decision.reason),
    "Stop Price": num(args.decision.stopPrice),
    "Risk Pct": num(args.decision.riskPct),
    "Initial Target": num(args.decision.targetPrice),
    "VNINDEX Entry": num(args.vnindex),
    ...
    "Last Rel Volume": num(args.decision.volumePace),
    "Max Favorable Pct": num(0),
    "Max Adverse Pct": num(0),
  }
  const response = await fetch("https://api.notion.com/v1/pages", {
    method: "POST",
    headers: headers(),
    body: JSON.stringify({ parent: { data_source_id: RECOMMENDATIONS_DATA_SOURCE_ID }, properties }),
    cache: "no-store",
  })
  const payload = await response.json()
...
export async function updateRecommendationMonitor(row: TradeRecommendation, quote: LiveQuote, exit: ExitDecision) {
  const properties: Record<string, any> = {
    ...
    "Last Price": num(quote.price),
    "Last Rel Volume": num(exit.volumePace),
    "Max Favorable Pct": num(exit.maxFavorablePct),
    "Max Adverse Pct": num(exit.maxAdversePct),
  }
  return patchPage(row.id, properties)
}
...
export async function closeRecommendation(row: TradeRecommendation, quote: LiveQuote, exit: ExitDecision, vnindex: number | null) {
  ...
  const properties: Record<string, any> = {
    ...
    "Sell Price": num(quote.price),
    "Sell Reason": rich(exit.reason),
    "Return Pct": num(exit.returnPct),
    "VNINDEX Exit": num(vnindex),
    "VNINDEX Return Pct": num(vnindexReturnPct),
    "Alpha Pct": num(alphaPct),
...
export async function createSignalEvent(args: { type: "BUY" | "SELL" | "EXIT_FAIL" | "WATCH"; recommendationId: string; scan: DailyScanRow | undefined; quote: LiveQuote; rule: string; relVolume: number | null; stopPrice: number | null; vnindex: number | null }) {
  ...
  const properties: Record<string, any> = {
    Signal: { title: [{ type: "text", text: { content: `${ticker} — ${args.type} — ${now.slice(0, 16)}` } }] },
    Ticker: rich(ticker),
    Type: { select: { name: args.type } },
    "Signal Time": { date: { start: now } },
    Price: num(args.quote.price),
    Volume: num(args.quote.totalVolume),
    ...
    "Stop Price": num(args.stopPrice),
    VNINDEX: num(args.vnindex),
  }
  const response = await fetch("https://api.notion.com/v1/pages", {
    method: "POST",
    headers: headers(),
    body: JSON.stringify({ parent: { data_source_id: SIGNAL_EVENTS_DATA_SOURCE_ID }, properties }),
    cache: "no-store",
  })
  const payload = await response.json()
