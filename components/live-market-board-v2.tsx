...
export function LiveMarketBoardV2({ universe }: { universe: BoardUniverseStock[] }) {
  ...
  const fallbackReferences = useMemo(() => Object.fromEntries(universe.map((stock) => [stock.ticker, stock.lastClose ?? 0])), [universe])
      ...
      try {
        ...
        socket.onmessage = (event) => {
          ...
          if (type === "t" && data.symbol) {
            ...
            if (!fallbackReferences[ticker]) return
            ...
            const reference = firstPositive(data, ["referencePrice", "refPrice", "basicPrice", "reference"]) || fallbackReferences[ticker]
          ...
          if (type === "mi") {
            ...
            const change = numeric(data.changedValue ?? data.change)
            const changePercent = numeric(data.changedRatio ?? data.changePercent)
  ...
  }, [symbolKey, reconnectKey, fallbackReferences, pushSparkPrice, symbolList])
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    ...
    <div className="flex items-center gap-4 border-b border-border px-4 py-2 text-[11px] text-muted-2"><span className="flex items-center gap-1.5"><Activity className="h-3.5 w-3.5" />Top 50 HOSE từ Notion</span><span>Tăng <b className="text-up">{advances}</b></span><span>Giảm <b className="text-down">{declines}</b></span><span>Có giá <b className="text-foreground">{liveCount}</b>/50</span>{lastMessageAt ? <span className="ml-auto">DNSE {new Date(lastMessageAt).toLocaleTimeString("vi-VN", { timeZone: "Asia/Ho_Chi_Minh" })}</span> : null}</div>
    <div className="min-h-0 flex-1 overflow-auto p-2.5">{mode === "sector" ? <div className="grid auto-cols-[330px] grid-flow-col gap-2.5">{grouped.map(({ sector, stocks }) => {
      ...
      return <section key={sector} className="flex max-h-full min-h-[240px] flex-col rounded-lg border border-border bg-panel"><header className="flex items-center justify-between border-b border-border px-3 py-2.5"><div><h2 className="text-sm font-semibold">{sector}</h2><p className="mt-0.5 text-[10px] text-muted-2">{stocks.length} mã</p></div><span className={`font-mono text-xs font-semibold ${boardPctClass(avg)}`}>{typeof avg === "number" ? `${avg > 0 ? "+" : ""}${avg.toFixed(2)}%` : "—"}</span></header><div className="flex-1 overflow-y-auto p-1.5">{stocks.map((stock) => <LiveStockRow key={stock.ticker} stock={stock} quote={quotes[stock.ticker] as LiveStockQuote | undefined} history={priceHistory[stock.ticker] ?? []} onOpen={() => openBook(stock.ticker)} />)}</div></section>
