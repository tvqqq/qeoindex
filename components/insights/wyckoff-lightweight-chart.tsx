...
function applyStudy(controller: ChartController, study: WyckoffChartStudy, showScenarios: boolean) {
...
export function WyckoffLightweightChart({
  ...
  embedded = false,
  showScenarios = true,
  showIntelligence: _showIntelligence = true,
}: {
  ticker: string
  ...
  const renderKey = useMemo(() => {
    ...
    const scenarioSignature = showScenarios ? study.scenarios.map((scenario) => `${scenario.key}:${scenario.probability}:${scenario.path.at(-1)?.value ?? ""}`).join(",") : "structure-only"
    return [ticker, study.timeframe, study.bars.length, firstBar?.time ?? 0, lastBar?.time ?? 0, lastBar?.open ?? 0, lastBar?.high ?? 0, lastBar?.low ?? 0, lastBar?.close ?? 0, lastBar?.volume ?? 0, study.analysis?.support ?? "", study.analysis?.resistance ?? "", study.markers.length, scenarioSignature].join("|")
  }, [showScenarios, study, ticker])
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const plusJakarta = getComputedStyle(document.documentElement).getPropertyValue("--font-plus-jakarta-sans").trim()
        const chartFontFamily = plusJakarta ? `${plusJakarta}, "Plus Jakarta Sans", ui-sans-serif, system-ui, sans-serif` : '"Plus Jakarta Sans", ui-sans-serif, system-ui, sans-serif'
        ...
        chart = lwc.createChart(host, {
          ...
          // Zoom stays disabled: continuous scale redraws can flicker on scaled 4K Chromium.
          handleScale: false,
        })
  ...
  useEffect(() => {
    ...
    updateFrameRef.current = requestAnimationFrame(() => {
      try {
        applyStudy(controller, study, showScenarios)
        setRuntimeError("")
  ...
  }, [chartReady, renderKey, showScenarios, study, ticker])
