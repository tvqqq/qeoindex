...
const WyckoffInfographicDashboard = dynamic(
  () => import("@/components/insights/wyckoff-infographic-dashboard").then((module) => module.WyckoffInfographicDashboard),
  { ssr: false },
)
...
export function WyckoffDeferredDashboard(props: {
  ...
  const loadSelectedTicker = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.ok || !payload.data) {
        throw new Error(payload.error || `Không tải được dữ liệu Wyckoff ${props.initialTicker}`)
      }
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      void (async () => {
        try {
          const response = await fetch("/api/insights/wyckoff?mode=watchlist", {
            signal: controller.signal,
            headers: { Accept: "application/json" },
          })
          const payload = await response.json() as WatchlistResponse
          if (response.ok && payload.ok && Array.isArray(payload.stocks) && !controller.signal.aborted) {
            setStocks(payload.stocks)
          }
        } catch {
          // Canonical 200 shell remains usable when phase hydration is temporarily unavailable.
        }
      })()
    })
        ...
        <div className="grid gap-4 xl:grid-cols-[500px_minmax(0,1fr)] 2xl:grid-cols-[520px_minmax(0,1fr)]">
          <Card className="hidden h-[calc(100vh-76px)] min-h-[660px] gap-0 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#090e15] py-0 ring-0 xl:flex">
            <CardContent className="w-full p-4">
              <div className="flex items-center justify-between border-b border-white/[0.06] pb-3">
                <div>
                  <div className="text-base font-extrabold text-white">Wyckoff Watchlist</div>
                  <div className="mt-1 text-xs font-semibold text-slate-500">Canonical Top Stocks · {stocks.length} mã</div>
                </div>
                <Radar className="size-5 text-cyan-300" />
              </div>
              <div className="mt-3 grid grid-cols-4 gap-2 text-xs font-bold text-slate-500">
                <span>Mã</span><span className="text-center">1H</span><span className="text-center">1D</span><span className="text-center">1W</span>
              </div>
              <div className="mt-2 space-y-1.5">
                {stocks.slice(0, 10).map((stock) => (
                  <div key={stock.ticker} className="grid h-10 grid-cols-4 items-center rounded-lg bg-white/[0.025] px-2 text-xs">
          ...
          <Card className="min-h-[560px] gap-0 rounded-2xl border border-white/[0.08] bg-[#0a1017] py-0 ring-0">
            <CardContent className="flex min-h-[560px] flex-col items-center justify-center p-6 text-center">
              {error ? (
                <>
                  <Radar className="size-10 text-rose-300" />
                  <h1 className="mt-4 text-lg font-extrabold text-white">Không tải được Wyckoff {props.initialTicker}</h1>
                  <p className="mt-2 max-w-xl text-sm font-medium text-slate-400">{error}</p>
                ...
                <>
                  ...
                  <h1 className="mt-4 text-lg font-extrabold text-white">Đang tải Wyckoff {props.initialTicker}</h1>
                  <p className="mt-2 text-sm font-medium text-slate-500">Shell canonical {stocks.length} mã đã render · chart tải sau paint.</p>
                </>
              )}
