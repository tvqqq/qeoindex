...
function probabilitiesFromScan(scan?: DailyScanRow): ProbabilitySet {
  return {
    bull: scan?.bullProbability ?? null,
    base: scan?.baseProbability ?? null,
    bear: scan?.bearProbability ?? null,
  }
}
...
function PriceHistoryChart({ ticker, bars }: { ticker: string; bars: OhlcvBar[] }) {
  ...
  const tickValues = [min, min + (max - min) / 2, max]
...
function MovingAverageRow({ label, ma, price }: { label: string; ma: number | null | undefined; price: number | null | undefined }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 border-t border-border/70 py-3 first:border-t-0 first:pt-0">
      <div><div className="font-medium text-foreground/75">{label}</div><div className="mt-1 font-mono text-sm text-foreground/55">{num(ma, 2)}</div></div>
      <div className={`font-mono text-sm font-semibold ${distance == null ? "text-foreground/50" : distance >= 0 ? "text-up" : "text-down"}`}>{distance == null ? "—" : `${distance >= 0 ? "+" : ""}${distance.toFixed(2)}%`}</div>
    </div>
  )
}
          ...
          <div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-start">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2 text-xs text-foreground/50">
                ...
                {row.timeframes.map((tf) => <span key={tf} className="rounded border border-border bg-panel-2 px-2 py-0.5">{tf}</span>)}
                {row.type.map((type) => <span key={type} className="rounded border border-border bg-panel-2 px-2 py-0.5">{type}</span>)}
              </div>
              <a href={row.notionUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1.5 text-base font-semibold text-foreground hover:text-brand">{row.analysis}<ExternalLink className="h-3.5 w-3.5" /></a>
...
export function StockDetailApp({
  ticker,
  thesis,
  scan,
  universe,
    ...
    <div className="min-h-screen bg-background text-[15px]">
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
            <div>
              <div className="flex flex-wrap items-center gap-2.5">
                <Link href="/research/scanner" className="inline-flex items-center gap-1.5 text-sm text-foreground/55 hover:text-brand"><ArrowLeft className="h-4 w-4" /> Scanner</Link>
                ...
                <span className={`rounded-md border px-2.5 py-1 text-xs font-semibold ${canonical ? "border-brand/35 bg-brand/10 text-brand" : "border-ref/35 bg-ref/10 text-ref"}`}>{canonical ? "Canonical Thesis" : "Scanner Candidate"}</span>
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-2.5">
                ...
                {universe && <span className="rounded-md border border-border bg-panel-2 px-2.5 py-1 text-xs text-foreground/55">Top 50 HOSE · #{universe.rank} · {num(universe.marketCapT)}T</span>}
              </div>
              {!canonical && <p className="mt-3 max-w-4xl text-sm leading-6 text-foreground/60">Trang này đang hiển thị scanner-level evidence. Đây chưa phải luận điểm đầu tư chuẩn hóa; chỉ nâng cấp thành canonical thesis sau review đa khung thời gian và xác nhận theo framework.</p>}
            </div>
            <div className="flex items-center gap-2">
              {previousTicker && <Link href={`/research/${previousTicker.toLowerCase()}`} className="rounded-md border border-border-strong bg-panel-2 px-3 py-2 text-sm text-foreground/65 hover:text-foreground">← {previousTicker}</Link>}
              {nextTicker && <Link href={`/research/${nextTicker.toLowerCase()}`} className="rounded-md border border-border-strong bg-panel-2 px-3 py-2 text-sm text-foreground/65 hover:text-foreground">{nextTicker} →</Link>}
            </div>
          </div>
        </div>
      </div>
      ...
      <main className="mx-auto max-w-[1600px] space-y-5 p-4 lg:p-6">
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_340px]">
            <div>
              ...
              <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-foreground/55">
                <span>Volume <strong className="font-mono font-semibold text-foreground/75">{num(scan?.volume, 0)}</strong></span>
                <span>Provider <strong className="font-semibold text-foreground/75">{scan?.provider || "—"}</strong></span>
                <span>Status <strong className="font-semibold text-foreground/75">{scan?.status || "—"}</strong></span>
                <span>Confidence <strong className="font-semibold text-foreground/75">{confidenceLabel(scan?.confidence || thesis?.confidence || "")}</strong></span>
              </div>
            </div>
            <div><div className="mb-3 text-sm font-semibold text-foreground/55">Xác suất {canonical ? "canonical" : "scanner"}</div><ScenarioBars probabilities={scenario} /></div>
          </div>
        </section>

        {vnindex && ticker !== "VNINDEX" && (
          <section className="rounded-xl border border-border bg-panel p-5">
            <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
              <div className="min-w-0"><div className="flex items-center gap-2"><span className="font-mono font-bold text-foreground">VNINDEX</span><span className="rounded-md border border-border-strong bg-panel-2 px-2 py-1 text-xs font-semibold text-foreground/65">{regimeLabel(vnindex.marketRegime)}</span></div><p className="mt-2 max-w-5xl text-sm leading-6 text-foreground/65">{vnindex.baseCase}</p></div>
              <Link href="/research/vnindex" className="shrink-0 text-sm font-medium text-brand">Mở market structure →</Link>
            </div>
          </section>
        )}
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="RSI14" value={num(scan?.rsi14, 1)} detail={scan?.rsi14 == null ? "Chưa có dữ liệu." : scan.rsi14 >= 70 ? "Vùng quá mua; cần đọc cùng structure/volume." : scan.rsi14 <= 30 ? "Vùng quá bán; chưa tự động đồng nghĩa đảo chiều." : "Momentum đang ở vùng trung tính."} icon={<Gauge className="h-5 w-5" />} />
          <MetricCard label="Relative Volume" value={scan?.relVolume == null ? "—" : `${scan.relVolume.toFixed(2)}x`} detail="So với baseline 20 phiên; volume là effort, không tự suy diễn ý chí tổ chức." icon={<BarChart3 className="h-5 w-5" />} />
          <MetricCard label="MACD" value={num(scan?.macd, 2)} detail={`Signal ${num(scan?.macdSignal, 2)}. Dùng làm evidence phụ, không thay thế cấu trúc giá.`} icon={<Activity className="h-5 w-5" />} />
          <MetricCard label="ATR14" value={num(scan?.atr14, 2)} detail="Biên độ dao động trung bình 14 phiên; hữu ích khi đánh giá khoảng cách tới support/invalidation." icon={<ShieldCheck className="h-5 w-5" />} />
        </div>

        <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
          <div className="space-y-5">
            <Section title="Wyckoff / Structure">{scan?.wyckoffState || thesis?.wyckoffState || "Chưa đủ dữ liệu để gắn cấu trúc."}<div className="mt-3 text-sm text-foreground/55">Phase: <span className="font-semibold text-foreground/70">{scan?.phase || "Unclassified"}</span></div></Section>
            <Section title="What changed" tone="warning">{scan?.whatChanged || thesis?.whatChanged || "Chưa có thay đổi được ghi nhận."}</Section>
            {thesis ? (
              <section className="rounded-xl border border-brand/25 bg-panel p-5">
                <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold text-foreground">Canonical investment thesis</h2><p className="mt-1 text-sm text-foreground/55">Trạng thái đã được review và lưu trong Stock Thesis.</p></div><a href={thesis.notionUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-medium text-brand">Notion <ExternalLink className="h-3.5 w-3.5" /></a></div>
                <div className="mt-5 grid gap-4 lg:grid-cols-2"><Section title="Base case">{thesis.baseCase || "—"}</Section><Section title="Canonical Wyckoff">{thesis.wyckoffState || "—"}</Section></div>
                <div className="mt-4 grid gap-4 lg:grid-cols-3"><Section title="Bull probability">{thesis.probabilities.bull == null ? "—" : `${thesis.probabilities.bull}%`}</Section><Section title="Base probability">{thesis.probabilities.base == null ? "—" : `${thesis.probabilities.base}%`}</Section><Section title="Bear probability">{thesis.probabilities.bear == null ? "—" : `${thesis.probabilities.bear}%`}</Section></div>
              </section>
            ) : (
              <section className="rounded-xl border border-ref/30 bg-ref/5 p-5"><div className="flex items-start gap-3"><Target className="mt-0.5 h-5 w-5 text-ref" /><div><h3 className="font-semibold text-foreground">Chưa có canonical thesis</h3><p className="mt-1.5 text-sm leading-6 text-foreground/65">Scanner chỉ tạo shortlist. Để trở thành thesis cần review Weekly/Daily/4H, volume behavior, VNINDEX context, confirmation/invalidation và nếu cần cả FA.</p></div></div></section>
            )}

            <section className="overflow-hidden rounded-xl border border-border bg-panel">
              <div className="border-b border-border px-5 py-4"><h2 className="text-lg font-semibold text-foreground">Analysis timeline</h2><p className="mt-1 text-sm text-foreground/55">Lịch sử phân tích được giữ nguyên để phục vụ hậu kiểm.</p></div>
          ...
          <div className="space-y-5">
            <section className="rounded-xl border border-border bg-panel p-5">
              ...
              <div className="mt-4"><MovingAverageRow label="MA20" ma={scan?.ma20} price={dailyPrice} /><MovingAverageRow label="MA50" ma={scan?.ma50} price={dailyPrice} /><MovingAverageRow label="MA200" ma={scan?.ma200} price={dailyPrice} /></div>
            </section>
        ...
        <div className="flex items-center justify-between border-t border-border pt-4 text-sm">
          <Link href="/research/scanner" className="inline-flex items-center gap-1.5 text-foreground/60 hover:text-brand"><ArrowLeft className="h-4 w-4" /> Quay lại Scanner</Link>
          <Link href="/research" className="inline-flex items-center gap-1.5 text-brand">Trung tâm Nghiên cứu <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </main>
    </div>
  )
