...
export function SignalsApp({ recommendations, events, performance, monitorReady, telegramReady, cronSecretReady, dataSource, readError, supabaseReady }: { recommendations: TradeRecommendation[]; events: SignalEventRow[]; performance: RecommendationPerformance; monitorReady: boolean; telegramReady: boolean; cronSecretReady: boolean; dataSource: "notion" | "supabase"; readError: string; supabaseReady: boolean }) {
  ...
  return <div className="min-h-screen bg-background text-[15px]"><TopNav />
    <main className="mx-auto max-w-[1700px] px-4 py-6 lg:px-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start"><div><div className="flex items-center gap-2"><BellRing className="h-5 w-5 text-brand"/><h1 className="text-2xl font-semibold text-foreground">Khuyến nghị & Signal Monitor</h1></div><p className="mt-2 max-w-4xl text-sm leading-6 text-foreground/55">Daily Bullish → intraday price/volume confirmation → BUY → open recommendation → monitor thesis-fail → SELL/EXIT_FAIL. Supabase là operational ledger; Notion là mirror và Telegram là notification channel.</p></div><Link href="/research/scanner" className="rounded-md border border-border bg-panel px-3 py-2 text-sm text-foreground/65 hover:text-brand">← Daily Scanner</Link></div>

      <div className="mt-5 flex flex-wrap gap-2 text-xs"><span className={`rounded border px-2.5 py-1 ${dataSource === "supabase" && supabaseReady ? "border-up/30 bg-up/10 text-up" : "border-ref/30 bg-ref/10 text-ref"}`}>Ledger · {dataSource}{dataSource === "supabase" && !supabaseReady ? " (missing env)" : ""}</span><span className={`rounded border px-2.5 py-1 ${monitorReady ? "border-up/30 bg-up/10 text-up" : "border-down/30 bg-down/10 text-down"}`}>DNSE live server · {monitorReady ? "configured" : "missing env"}</span><span className={`rounded border px-2.5 py-1 ${telegramReady ? "border-up/30 bg-up/10 text-up" : "border-ref/30 bg-ref/10 text-ref"}`}>Telegram · {telegramReady ? "configured" : "pending config"}</span><span className={`rounded border px-2.5 py-1 ${cronSecretReady ? "border-up/30 bg-up/10 text-up" : "border-ref/30 bg-ref/10 text-ref"}`}>Cron auth · {cronSecretReady ? "ready" : "pending"}</span><span className="rounded border border-down/30 bg-down/10 px-2.5 py-1 font-semibold text-down">Scheduler 1 phút · CHƯA ACTIVE</span><span className="rounded border border-border bg-panel-2 px-2.5 py-1 text-foreground/55">Engine intraday-v1.0 · fail-closed</span></div>
      {readError && <div className="mt-3 rounded-lg border border-down/30 bg-down/10 px-4 py-3 text-xs text-down">Không đọc được {dataSource} operational ledger: {readError}</div>}
      <div className="mt-3 rounded-lg border border-ref/25 bg-ref/5 px-4 py-3 text-xs leading-5 text-foreground/60"><strong className="text-ref">Infrastructure gate:</strong> endpoint monitor đã sẵn sàng nhưng chưa có scheduler near-live được xác minh. StockOS sẽ không hiển thị trạng thái đang monitor cho đến khi job 1 phút thực sự được kích hoạt và quan sát thành công.</div>
