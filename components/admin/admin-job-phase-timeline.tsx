...
export function AdminJobPhaseTimeline({ rows }: AdminJobPhaseTimelineProps) {
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-white/[0.08] bg-[#0c1017] p-5 sm:p-6" aria-label="QeoIndex EOD pipeline phases">
      <div className="flex flex-col gap-2 border-b border-white/[0.06] pb-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Pipeline Execution Phases</h3>
            <p className="text-[11px] text-slate-400">
              Dependency-driven: phase sau chỉ chạy khi phase trước hoàn tất hợp lệ.
            </p>
          </div>
        </div>
        <span className="rounded-full border border-white/[0.08] bg-white/[0.03] px-2.5 py-0.5 font-mono text-[10px] text-slate-400">
          {rows.length ? `${rows.length} phase telemetry events` : "Chưa có phase telemetry"}
        </span>
      </div>
        ...
        <ol className="divide-y divide-white/[0.05]">
          {phases.map((phase) => {
            ...
            return (
              <li
                key={phase.key}
                className="grid gap-3.5 p-4 transition-colors hover:bg-white/[0.01] lg:grid-cols-[40px_minmax(220px,1fr)_minmax(260px,1.5fr)]"
              >
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-x-3.5 gap-y-1 font-mono text-[10px] text-slate-400">
                    <span>Thời lượng: <strong className="text-slate-200">{formatDuration(phase.durationMs)}</strong></span>
                    {phase.startedAt ? <span>Bắt đầu (ICT): <strong className="text-slate-200">{formatAdminDateTime(phase.startedAt)}</strong></span> : null}
                  </div>
                </div>

                <div className="min-w-0">
                  {phase.errorMessage ? (
                    <div className="rounded-xl border border-rose-500/25 bg-rose-500/10 p-3 text-[11px] text-rose-300">
                    ...
                    <dl className="grid gap-2 sm:grid-cols-2">
                      {summaryEntries.map(([key, value]) => (
                        <div key={key} className="min-w-0 rounded-xl border border-white/[0.06] bg-[#0c1017] p-2.5">
                          <dt className="truncate font-mono text-[9px] uppercase tracking-wider text-slate-400">{key}</dt>
                          <dd className="mt-0.5 break-words font-mono text-[10px] font-semibold text-slate-200">{formatSummaryValue(value)}</dd>
                        </div>
                      ))}
                    </dl>
                  ) : (
