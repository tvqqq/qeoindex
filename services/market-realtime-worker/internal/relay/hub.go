...
func (h *Hub) newClient(subject string, onSlow func()) *client {
	return &client{subject: subject, send: make(chan []byte, h.sendQueue), done: make(chan struct{}), topics: map[string]struct{}{}, onSlow: onSlow}
}
...
func (h *Hub) Stats() HubStats {
	h.mu.RLock()
	defer h.mu.RUnlock()
	stats := HubStats{Clients: len(h.clients), SlowConsumers: h.slowConsumers.Load(), Published: h.published.Load(), PublishedBytes: h.publishedBytes.Load()}
