      ...
      {row.evidenceProvenance ? (
        <section className="mx-4 mt-4 rounded-2xl border border-cyan-400/10 bg-cyan-400/[0.025] p-3.5">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-7">
            ...
            <div className="rounded-xl border border-white/[0.06] bg-black/15 p-2.5"><div className="text-[8px] font-black uppercase text-slate-600">Ticker Knowledge</div><p className="mt-1 font-mono text-[9px] text-slate-400">{shortHash(row.evidenceProvenance.tickerKnowledgeContextHash)}</p><p className="mt-0.5 text-[8px] text-slate-600">{row.evidenceProvenance.tickerKnowledgeStatus ? `${row.evidenceProvenance.tickerKnowledgeStatus} · ${row.evidenceProvenance.tickerKnowledgeCount} items` : "not frozen"}</p><p className="mt-0.5 truncate text-[8px] text-slate-700">{row.evidenceProvenance.tickerKnowledgeContextVersion || "—"} · retrieval {row.evidenceProvenance.tickerKnowledgeRetrievalStatus || "—"} · {row.evidenceProvenance.tickerKnowledgeCapturedAt || "—"}</p></div>
            <div className="rounded-xl border border-white/[0.06] bg-black/15 p-2.5"><div className="text-[8px] font-black uppercase text-slate-600">OpenAI cache identity</div><p className="mt-1 font-mono text-[9px] text-cyan-300">{shortHash(row.evidenceProvenance.promptIdentityHash)}</p><p className="mt-0.5 text-[8px] text-slate-600">deterministic + raw + research + reports + ticker knowledge + prompt</p></div>
          </div>
        </section>
      ) : null}
