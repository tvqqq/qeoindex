...
function failure(error: unknown) {
  ...
  console.error("[Portfolio Risk Plan] risk-profile request failed", error)
  return NextResponse.json({ ok: false, error: "Risk Profile request failed." }, { status: 500, headers: NO_STORE })
}
...
export async function GET(
  ...
  try {
    const attempts = await listRiskProfileAttempts(auth.context, id)
...
export async function POST(
  ...
  try {
    const attempt = await createRiskProfileAttempt(auth.context, id, body)
