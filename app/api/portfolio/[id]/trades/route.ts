...
import { createTrade, listTrades } from "@/modules/portfolio/trades/server"
...
function failure(error: unknown) {
  if (error instanceof TradeDomainError) {
    const status = error.code === "NOT_FOUND" ? 404 : ["INVALID_TRANSITION", "FROZEN_INITIAL_SNAPSHOT"].includes(error.code) ? 409 : 400
    return NextResponse.json({ ok: false, error: error.message, code: error.code }, { status, headers: NO_STORE })
  }
  console.error("[Portfolio Trades] request failed", error)
...
export async function GET(
  ...
  try {
    const trades = await listTrades(auth.context, id)
    return NextResponse.json({ ok: true, trades }, { headers: NO_STORE })
  } catch (error) {
...
export async function POST(
  ...
  try {
    const trade = await createTrade(auth.context, id, body)
    return NextResponse.json({ ok: true, trade }, { status: 201, headers: NO_STORE })
  } catch (error) {
    return failure(error)
  }
}
