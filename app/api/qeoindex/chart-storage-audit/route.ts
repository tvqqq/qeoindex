...
import { runProductionChartStorageAudit } from "@/modules/market/chart-data/storage-audit-server"
...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await runProductionChartStorageAudit(supabase, ticker)
    return NextResponse.json({
      ok: result.passed,
      mode: "chart-storage-audit",
      result,
    }, {
    ...
    console.error("[qeo231-chart-storage-audit]", error instanceof Error ? error.message : String(error))
    return NextResponse.json({
      ok: false,
      mode: "chart-storage-audit",
      error: "Chart storage audit unavailable.",
    }, {
      status: 500,
      headers: { "Cache-Control": "private, no-store" },
    })
